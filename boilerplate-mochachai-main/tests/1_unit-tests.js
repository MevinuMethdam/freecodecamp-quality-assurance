...
suite('Unit Tests', function () {
  ...
  suite('Comparisons', function () {
    ...
    test('#approximately', function () {
      assert.approximately(7 / 3, 2.33, 0.01);
  ...
  suite('Arrays', function () {
    ...
    test('Array #include, #notInclude', function () {
      assert.notInclude(['chocolate', 'vanilla', 'strawberry'], 'cinnamon', "don't have cinnamon");
      assert.include([1, 2, 3], 2, 'has 2');
    });
  });
  ...
  suite('Strings', function () {
    ...
    test('String #include, #notInclude', function () {
      assert.include('Home Sweet Home', 'Home');
      assert.include('JavaScript', 'Java', 'should contain Java');
    });
    // #15
   test('#match, #notMatch', function () {
      const regex = /^#\s*(\w+)/;
      assert.notMatch(' #-Hello', regex);
